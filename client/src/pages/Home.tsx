...
function DirectorateDistributionCard({ title, icon: Icon, data, tone, rule }: { title: string; icon: typeof Users; data: { name: string; value: number }[]; tone: string; rule: string }) {
  ...
  return <Card className="min-w-0 border-0 bg-white/95 shadow-sm"><CardHeader className="pb-1"><CardTitle className="flex items-center gap-2 text-sm"><Icon className="h-4 w-4" style={{ color: tone }} /> {title}</CardTitle><p className="text-[11px] text-slate-400">{rule} — عدد الموظفين الحاليين فقط</p></CardHeader><CardContent className="h-64 overflow-y-auto pr-1">{data.length ? <div style={{ height: `${chartHeight}px` }} dir="ltr"><SafeChart><BarChart layout="vertical" data={data} margin={{ top: 4, right: 20, left: 4, bottom: 4 }}><CartesianGrid horizontal={false} stroke="#e2e8f0" /><XAxis type="number" tick={{ fontSize: 9 }} axisLine={false} tickLine={false} /><YAxis type="category" dataKey="name" width={200} tick={{ fontSize: 9 }} axisLine={false} tickLine={false} interval={0} /><Tooltip formatter={(value: number) => [nf.format(value), "عدد الموظفين"]} /><Bar dataKey="value" name="عدد الموظفين" fill={tone} radius={[0, 4, 4, 0]} /></BarChart></SafeChart></div> : <div className="flex h-full items-center justify-center text-xs text-slate-400">لا توجد مديريات ضمن نطاق الفلتر الحالي.</div>}</CardContent></Card>;
}
