...
export default function Home() {
  ...
  const movementChart = metrics.newContractsByMonth.map((row: any, index: number) => ({ month: row.month, contracts: row.value, separations: metrics.separationsByMonth[index]?.value || 0, transfersIn: metrics.transfersInByMonth[index]?.value || 0, transfersOut: metrics.transfersOutByMonth[index]?.value || 0, transfersInternal: metrics.transfersInternalByMonth[index]?.value || 0 }));
