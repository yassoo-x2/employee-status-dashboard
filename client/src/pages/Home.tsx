...
import { buildFilteredMovementChart, countFilteredDirectorates, filterPublicUpdates, PUBLIC_FIXED_WIDGET_LABEL } from "@shared/public-filter-utils";
...
export default function Home() {
  ...
  const directorateCounts = countFilteredDirectorates(filtered);
  const latestUpdates = filtered.map((record: any) => record.latestUpdate).filter(Boolean);
  const scopedUpdates = filterPublicUpdates(data.updates ?? [], { month, status, organization, department });
  ...
  const movementChart = buildFilteredMovementChart(scopedUpdates, metrics.newContractsByMonth.map((row: any) => row.month), { month: "all" });
  ...
  return <DashboardLayout><div dir="rtl" className="min-h-[calc(100vh-2rem)] bg-[#f5f1e8] p-3 text-slate-900 lg:p-4">
    ...
    <p className="mt-2 text-center text-[11px] text-slate-400">لوحة إحصائية مجمّعة — لا تعرض أسماء أو سجلات فردية. الرواتب والمراكز والخريطة وحضور التدريب إجماليات مصدرية غير متأثرة بالفلاتر.</p>
  </div></DashboardLayout>;
}
